...
struct DailyIntakeData {
    ...
    static let dailyCarbsIntake = 260.0 // grams
    static let maxDailyCarbs = 30.0
    static let minDailyCarbs = 15.0
    
    static let dailySugarsInatke = 90.0 // grams
    static let maxDailySugars = 22.5
    static let minDailySugars = 5.0
    
    static let dailyProteinIntake = 50.0 // grams
    static let maxDailyProtein = 10.0
    static let minDailyProtein = 5.0
...
// **************** Variables **************** \\
    ...
    var carbsRV = ""
    var carbs = 0.0
    var carbsColor = Color.white
    var proteinV = ""
    var proteinRV = ""
    var protein = 0.0
    var proteinColor = Color.white
    ...
    func getAll(energy : Double, fat : Double, carbs : Double, protein : Double, saturate : Double, sugar : Double, salt : Double, serv_qty : Double) {
        ...
        calculateCarbs(carbs: carbs, srv_qty: srv_qty)
        calculateProtein(protein: protein, srv_qty: srv_qty)
    ...
    // Works out the Carbs percentage for daily intake and the color based on the min/max daily intake
    func calculateCarbs(carbs : Double, srv_qty : Double) {
        carbsV = "\((carbs).roundToDecimal(2))g"
        let carbsF = (((carbs*srv_qty) / DailyIntakeData.dailyTotalFatIntake)*100).roundToDecimal(2)
        self.carbs = carbs
        carbsRV = ("\(carbsF)%")
        if carbs <= DailyIntakeData.minDailyCarbs {
            carbsColor = Color.green
        } else if carbs >= DailyIntakeData.maxDailyCarbs{
            carbsColor = Color.red
        } else {
            carbsColor = Color.orange
        }
    }
    
    // Works out the Protein percentage for daily intake and the color based on the min/max daily intake
    func calculateProtein(protein : Double, srv_qty : Double) {
        proteinV = "\((protein).roundToDecimal(2))g"
        let proteinF = (((protein*srv_qty) / DailyIntakeData.dailyProteinIntake)*100).roundToDecimal(2)
        self.protein = protein
        proteinRV = ("\(proteinF)%")
        if protein <= DailyIntakeData.minDailyProtein {
            proteinColor = Color.green
        } else if protein >= DailyIntakeData.maxDailyProtein{
            proteinColor = Color.red
        } else {
            proteinColor = Color.orange
        }
    }
