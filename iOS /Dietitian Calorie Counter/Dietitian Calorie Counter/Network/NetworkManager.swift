...
class NetworkManager : ObservableObject {
    ...
    // https://trackapi.nutritionix.com/v2/search/instant?branded_region=2&query=cak&x-app-id=648ee0b1&x-app-key=ea98f601939116b015738b551c48ecab
    func createRequest(request : URL) -> URLRequest {
        var request  = URLRequest(url : request)
        request.addValue("648ee0b1", forHTTPHeaderField: "x-app-id")
        request.addValue("ea98f601939116b015738b551c48ecab", forHTTPHeaderField : "x-app-key")
    ...
    func performQuery(query : String) {
        ...
        print("finalURL:",finalURL as Any)
    ...
    func getCommonFoodItemDetails(food_name : String, callback: @escaping () -> Void) {
        ...
        URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            FoodEnergy.shared.getAll(energy: self.foodDetail.first?.nf_calories ?? 0.0, 
                                     fat: self.foodDetail.first?.nf_total_fat ?? 0.0,
                                     carbs: self.foodDetail.first?.nf_total_carbohydrate ?? 0.0,
                                     protein: self.foodDetail.first?.nf_protein ?? 0.0,
                                     saturate: self.foodDetail.first?.nf_saturated_fat ?? 0.0,
                                     sugar: self.foodDetail.first?.nf_sugars ?? 0.0,
                                     salt: self.foodDetail.first?.nf_sodium ?? 0.0,
                                     serv_qty: self.foodDetail.first?.serving_weight_grams ?? 0.0)
    ...
    func getBrandedFoodItemDetails(food_name : String, callback: @escaping () -> Void) {
        ...
        let details : Foods = Foods(food_name: "", brand_name: "", photo: Photo(thumb: ""), nf_calories: 0, nf_total_fat: 10.0, nf_total_carbohydrate: 10.0, nf_protein: 10.0, nf_saturated_fat: 3.0, nf_sugars: 2.0, nf_sodium: 2.0, serving_weight_grams: 2.0)
